...
export default function Dry() {
    ...
    //Product information
    const [cabin, setCabin] = useState('dry');       //when product added directly to ´dry´, cabin state is automatically set on ´dry´
    ...
    // others
    const [dry, setDry] = useState([]);             // for listing the products on this page
    ...
    const saveItem = () => {
        if (product === null) {
            ...
            Alert.alert(product + ' added to Dry!');        // products added succesfully
    ...
    const saveEdit = (e) => {
        ...
        setEdit(false);     //closing edit dialog
    }
    ...
    // Reading data from database that has 'dry' as 'cabin' information.
    // Setting data to 'setDry' Arraylist
    useEffect(() => {
        firebase.database().ref('kitchen/').orderByChild('cabin').equalTo('dry').on('value', snapshot => {
            if (snapshot.exists()){
